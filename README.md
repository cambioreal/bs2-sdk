...
(que só registrava `POST create` → `400 CT02098: Client not located in context`). Nenhum teste de
sandbox real além da autenticação pode avançar até a BS2 provisionar este client contra um
contexto de conta válido — ação externa, não resolvível por código. Ver
